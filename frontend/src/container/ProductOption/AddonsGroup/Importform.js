...
import "../option.css";
import {
  ImportAddonGroupInBulk,
  ConfirmImport,
} from "../../../redux/AddonGroup/actionCreator";

const { Dragger } = Upload;

const ImportAddonGroup = () => {
  ...
  const csvData = [["Addon Group Name", "Addon Name", "Sort Order"]];
  ...
  const handleSubmit = async (formData) => {
    ...
    let savedimportfile = await dispatch(ImportAddonGroupInBulk(formdata));
  ...
  if (showPreview === true) {
    if (PreviewData.length) {
      PreviewData.map((value) => {
        const { addon_group_name, addon_name, sort_order } = value.record;
  ...
  const columns = [
    {
      ...
      render: (data, record) => (
        <p>{record.isShow === true ? record.addon_group_name : ""}</p>
      ),
    },
    {
      title: "Addon Name",
        ...
        {showMessage === false ? (
          showPreview === false ? (
            <PageHeader
              ...
              ghost
              title="Import Addon Group"
              buttons={[
                <div key="1" className="page-header-actions">
                  <CSVLink
                    filename="Import-AddonGroup-Template.csv"
                    data={csvData}
...
export default ImportAddonGroup;
