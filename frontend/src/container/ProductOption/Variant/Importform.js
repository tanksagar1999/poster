...
const ImportVariant = () => {
  ...
  const csvData = [["Variant Name", "Variant Comment", "Price", "Sort Order"]];
  ...
  const handleSubmit = async (formData) => {
    ...
    let savedimportfile = await dispatch(ImportVariantInBulk(formdata));
  ...
  if (showPreview === true) {
    if (PreviewData.length) {
      PreviewData.map((value) => {
        const { variant_name, comment, price, sort_order } = value.record;
  ...
  const columns = [
    {
      ...
      key: "variant_name",
      render: (data, record) => (
        <p>{record.isShow === true ? record.variant_name : ""}</p>
      ),
    },
    {
      ...
      key: "comment",
      render: (data, record) => (
        <p>{record.isShow === true ? record.comment : ""}</p>
      ),
    },
    {
      title: "Price",
            ...
            <PageHeader
              ...
              buttons={[
                <div key="1" className="page-header-actions">
                  <CSVLink
                    filename="Import-Variant-Template.csv"
                    data={csvData}
