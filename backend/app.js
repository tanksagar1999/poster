...
//Added by bc for receipt number user
app.use(function (req, res, next) {
  // check if client sent cookie
  var cookie = req.headers.receiptnumber;
  console.log('HEADER COOKIES DATA == >',cookie);
  if (!cookie || cookie === undefined) {
    // no: set a new cookie
    var randomNumber=Math.random().toString();
    randomNumber=randomNumber.substring(2,randomNumber.length);
    res.cookie('id',randomNumber, { maxAge: 900000, httpOnly: true });
    //res.headers.random = randomNumber;
    res.setHeader('id', randomNumber);
   // res.setHeader('Access-Control-Allow-Credentials', true);
   // res.setHeader('Access-Control-Allow-Origin', 'http://172.105.35.50');
